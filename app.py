...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "change-cette-cle-en-production")
...
# --------------------------------------------------------------------
# Compteur de visites (stocké dans un simple fichier JSON).
# ⚠️ Sur Render (plan gratuit), le disque est éphémère : ce fichier
# peut être remis à zéro après un redéploiement ou une longue période
# d'inactivité. Pour un compteur permanent, il faudrait une vraie base
# de données (ex : Render PostgreSQL).
# --------------------------------------------------------------------
VISITS_FILE = "visits.json"
 
 
def get_visit_count():
    """Lit le nombre de visites depuis le fichier, ou 0 si absent/corrompu."""
...
def increment_visit_count():
    """Augmente le compteur de 1 et sauvegarde dans le fichier."""
...
@app.before_request
def count_visit():
    """Compte uniquement les visites de la page d'accueil (GET /)."""
